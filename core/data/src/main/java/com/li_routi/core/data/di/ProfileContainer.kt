package com.li_routi.core.data.di

import com.li_routi.core.data.network.NetworkModule
import com.li_routi.core.data.repository.ProfileRepositoryImpl
import com.li_routi.core.domain.profile.ProfileRepository
import com.li_routi.core.domain.profile.UpdateProfileUseCase

/**
 * Hilt 등 DI 프레임워크가 붙기 전까지 사용하는 수동 구성 root.
 * feature 모듈은 여기서 필요한 UseCase만 가져다 쓴다.
 */
object ProfileContainer {

    private val repository: ProfileRepository by lazy {
        ProfileRepositoryImpl(
            memberApi = NetworkModule.memberApiService,
            mediaApi = NetworkModule.mediaApiService,
            uploadHttpClient = NetworkModule.uploadOkHttpClient,
        )
    }

    val updateProfileUseCase: UpdateProfileUseCase by lazy {
        UpdateProfileUseCase(repository)
    }
}
