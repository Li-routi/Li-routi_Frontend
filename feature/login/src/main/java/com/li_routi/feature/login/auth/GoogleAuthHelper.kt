...
class GoogleAuthHelper(
    ...
    /**
     * @param nonce 서버(`/api/auth/google/nonce`)가 발급한 1회용 값을 그대로 전달한다.
     *   서버가 ID 토큰의 nonce 클레임을 검증하므로 클라이언트에서 별도로 해싱하지 않는다.
     */
    suspend fun getGoogleIdToken(activity: Activity, nonce: String): String {
